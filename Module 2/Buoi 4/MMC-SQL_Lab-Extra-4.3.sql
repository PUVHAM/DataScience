...
-- Lưu ý là trong SQL server thì chúng ta có thể sử dụng trực tiếp FULL OUTER JOIN nhưng chúng ta đang sử dụng xampp và xampp sẽ cài đặt MySQL thay vì SQL server do đó  ta phải sử dụng LEFT OUTER JOIN kết hợp với RIGHT OUTER JOIN để thực hiện FULL OUTER JOIN
